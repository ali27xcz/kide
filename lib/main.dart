...
void main() async {
  ...
  // Start app immediately, then preload Lottie animations in background
        ...
        builder: (context, languageProvider, child) {
          ...
          return MaterialApp(
            ...
            theme: ThemeData(
              ...
              // اجعل خلفية الـ Scaffold شفافة لتفادي وميض خلفية بيضاء بين الشاشات
              scaffoldBackgroundColor: Colors.transparent,
              // ألغِ أي انتقالات افتراضية قد تسبب فريم أبيض على بعض الأجهزة
            ...
            // عرض Splash مع إعداد الصفحة الرئيسية في الخلفية لتجنب الشاشة البيضاء
            home: const _AppHome(),
            // غطاء خلفية عام متدرّج مطابق لمنع أي ومضة بيضاء بين أول إطارات
...
// Transition builder without animations to avoid white flash between routes
...
// App home wrapper that manages smooth transition from splash to main content
...
class _AppHomeState extends State<_AppHome> {
  ...
  void _initializeApp() {
    // Set ready immediately - no artificial delays
    setState(() {
      _isReady = true;
    });
    
    // Preload Lottie animations in background after first frame
    WidgetsBinding.instance.addPostFrameCallback((_) {
      unawaited(LottiePreloader.preloadAnimations());
    });
    
    // Switch after 7 seconds
  ...
  Widget build(BuildContext context) {
    if (!_isReady) {
      // Show a simple splash while initializing
      return const SplashScreen(renderOnly: true);
    }
    
    return Consumer<AuthProvider>(
      builder: (context, authProvider, child) {
        final bool isLoggedIn = authProvider.isLoggedIn;
        final Widget targetScreen = isLoggedIn ? const HomeScreen() : const LoginScreen();
        
        if (_showSplash) {
          // Only show splash - don't pre-build heavy screens
          return const SplashScreen(renderOnly: true);
        } else {
          return targetScreen;
        }
