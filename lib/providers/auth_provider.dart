...
class AuthProvider extends ChangeNotifier {
  ...
  String _cleanErrorMessage(Object e) {
    ...
    // Normalize common unexpected texts
    if (msg.contains('البريد الإلكتروني أو كلمة المرور غير صحيحة') ||
  ...
  void _initializeAuth() {
    ...
    // Listen to auth state to keep session persistent across app restarts
    try {
  ...
  Future<bool> signUp({
    ...
    try {
      ...
      if (_authService!.currentUser != null) {
        _currentUser = _authService!.currentUser;
        print('Sign up considered successful (user present) despite non-fatal error');
      } else {
  ...
  Future<bool> signIn({
    ...
    try {
      ...
      // Otherwise show a clear and professional message from service
      _setError(_cleanErrorMessage(e));
      _setLoading(false);
      return false;
    }
    ...
    try {
      await _loadParentProfile();
      // Sync cloud data into local cache after successful login
      try {
        final storage = await LocalStorageService.getInstance();
        await storage.syncLocalToCloudOnLogin(_currentUser!.uid);
        await storage.syncFromCloud(_currentUser!.uid);
      } catch (e) {
        print('Warning syncing data on login: $e');
      }
    } catch (e) {
