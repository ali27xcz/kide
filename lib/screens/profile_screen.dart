...
class _ProfileScreenState extends State<ProfileScreen>
  ...
  // Removed motivational phrases per requirement
  ...
  // Will auto-load from AssetManifest; start empty (no hardcoded filenames)
  List<String> _assetAvatarOptions = [];

  // Options: use ONLY local assets placed by the user
  List<String> get _avatarOptions => _assetAvatarOptions;
  ...
  Future<void> _loadAvatarAssets() async {
    try {
      ...
      setState(() {
        ...
        if (_selectedAvatar.isEmpty && _assetAvatarOptions.isNotEmpty) {
          _selectedAvatar = _assetAvatarOptions[Random().nextInt(_assetAvatarOptions.length)];
        }
      });
    } catch (_) {
  ...
  void initState() {
    ...
    if (widget.isCreating) {
      _isEditing = true;
      _isLoading = false;
      // pick random available avatar if exists else stay empty until assets load
      ...
      return SizedBox.expand(
        child: Image.asset(
          ...
          errorBuilder: (context, error, stackTrace) {
            return Image.asset('assets/images/avatars/child.png', fit: BoxFit.cover);
          },
        ),
      );
    }
    if (value.startsWith('http')) {
      ...
      return SizedBox.expand(
        child: Image.network(
          value,
          fit: BoxFit.cover,
          errorBuilder: (_, __, ___) => Image.asset('assets/images/avatars/child.png', fit: BoxFit.cover),
        ),
      );
    }
    // If it's neither asset nor URL, always use our default asset avatar
    return SizedBox.expand(child: Image.asset('assets/images/avatars/child.png', fit: BoxFit.cover));
  }
    ...
    try {
      ...
      if (_childProfile != null) {
        ...
        _selectedAvatar = _childProfile!.avatarPath.isNotEmpty 
            ...
            : (_avatarOptions.isNotEmpty ? _avatarOptions.first : '');
      }
  ...
  Future<void> _initializeStorageData() async {
    try {
      _storage = await LocalStorageService.getInstance();
      final profileFuture = _storage!.getChildProfile();
      final achievementsFuture = _storage!.getAchievements();
      
      final results = await Future.wait([profileFuture, achievementsFuture]);
      _childProfile = results[0] as ChildProfile?;
  ...
  void _startPhraseRotation() {
    // no-op: motivational phrases removed for profile screen
  }
  ...
  Widget _buildEditForm() {
    return Column(
      children: [
        // Keep only the avatar without motivational phrases
        SizedBox(
          height: 160,
      ...
      decoration: BoxDecoration(
        gradient: const LinearGradient(
          colors: [
            ...
            Color(0xFFF7FAFF), // very light bluish-white bottom
          ],
          begin: Alignment.topLeft,
          end: Alignment.bottomRight,
        ),
  ...
  void _saveProfile() async {
    ...
    try {
      final profile = _childProfile?.copyWith(
        ...
        // إنشاء ملف مرتبط مباشرة بحساب المستخدم عبر LocalStorageService ليحفظ في السحابة إن توفر
        ...
        return ChildProfile(
          ...
          age: age,
          avatarPath: _selectedAvatar,
          parentId: '', // سيتم ضبطه داخل saveChildProfile حسب uid الحالي
        );
      })();
