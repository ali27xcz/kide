...
class _PhonicsBlendingGameScreenState extends State<PhonicsBlendingGameScreen> {
  ...
  Future<void> _finish() async {
    ...
    if (!mounted) return;
    try {
      final success = 100 > 0 ? (_score / 100) >= AppConstants.passScoreRatio : false;
      await _audioService?.playGameEndSequence(success);
    } catch (_) {}
              ...
              Padding(
                ...
                child: Row(
                  children: [
                    ...
                    const Text('دمج الأصوات', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
                    const Spacer(),
                    Text('Score: $_score'),
                  ],
                ),
              ...
              Container(
                ...
                child: Column(
                  children: [
                    ...
                    // عرض الهدف LTR دائماً لتجنّب انقلاب ترتيب الأحرف في الواجهة العربية
                    Directionality(textDirection: TextDirection.ltr, child: Text(target, style: const TextStyle(fontSize: 28, fontWeight: FontWeight.bold))),
                  ],
                ),
              ),
              Wrap(
